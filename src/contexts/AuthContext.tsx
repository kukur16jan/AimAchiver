...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (emailOrUsername: string, password: string): Promise<boolean> => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://aim-achiever-backend.onrender.com/api/login', {
        method: 'POST',
  ...
  const loginWithGoogle = async (): Promise<boolean> => {
    ...
    try {
      ...
      const res = await fetch('https://aim-achiever-backend.onrender.com/auth/user', {
        credentials: 'include',
      });
      const data = await res.json();
      if (res.ok && data.user) {
  ...
  const register = async (name: string, email: string, password: string): Promise<string | null> => {
    ...
    try {
      ...
      const res = await fetch('https://aim-achiever-backend.onrender.com/api/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const logout = async () => {
    ...
    try {
      await fetch('https://aim-achiever-backend.onrender.com/auth/logout', {
        method: 'GET',
        credentials: 'include',
      });
    } catch {
