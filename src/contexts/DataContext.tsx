...
export const DataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchData = async () => {
      if (user) {
        const [tasksRes, moodsRes, peersRes] = await Promise.all([
          ...
          fetch(`http://localhost:3000/api/moods/${user.id}`),
          fetch(`http://localhost:3000/api/peers?userId=${user.id}`)
        ]);
  ...
  const addMoodEntry = async (moodData: Omit<MoodEntry, 'id'> & { moodInput?: string }) => {
    ...
    const res = await fetch('http://localhost:3000/api/moods/gemini', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const invitePeer = async (email: string) => {
    await fetch(`http://localhost:3000/api/peers/invite`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recipientEmail: email, requesterId: user?.id })
    });
    // Refresh peers
    const res = await fetch(`http://localhost:3000/api/peers?userId=${user?.id}`);
  ...
  const removePeer = async (peerId: string) => {
    await fetch(`http://localhost:3000/api/peers/${peerId}?userId=${user?.id}`, { method: 'DELETE' });
    // Refresh peers
  ...
  const acceptPeer = async (token: string) => {
    await fetch(`http://localhost:3000/api/peers/accept/${token}?userId=${user?.id}`);
    // Refresh peers
    const res = await fetch('http://localhost:3000/api/peers');
    const peersData = await res.json();
    setPeers(peersData.map((p: any) => ({ ...p, id: p._id })));
